...
def get_summary(req: SummaryRequest):
    print(req.requests)
    assert len(req.requests[0].reviews) == 8
    assert len(req.requests[1].reviews) == 8
    req1 = req.requests[0]
    req2 = req.requests[1]
    
    df = pd.DataFrame(
        {
    "cat": [req1.category, req2.category],
    "group_id": [req1.group_id, req2.group_id],
    "rev1": [req1.reviews[0], req2.reviews[0]],
    "rev2":[req1.reviews[1], req2.reviews[1]],
    "rev3":[req1.reviews[2], req2.reviews[2]],
    "rev4":[req1.reviews[3], req2.reviews[3]],
    "rev5":[req1.reviews[4], req2.reviews[4]],
    "rev6":[req1.reviews[5], req2.reviews[5]],
    "rev7":[req1.reviews[6], req2.reviews[6]],
    "rev8":[req1.reviews[7], req2.reviews[7]],
    "rating1":[req1.rating[0], req2.rating[0]],
    "rating2":[req1.rating[1], req2.rating[1]],
    "rating3":[req1.rating[2], req2.rating[2]],
    "rating4":[req1.rating[3], req2.rating[3]],
    "rating5":[req1.rating[4], req2.rating[4]],
    "rating6":[req1.rating[5], req2.rating[5]],
    "rating7":[req1.rating[6], req2.rating[6]],
    "rating8":[req1.rating[7], req2.rating[7]],
    "summ1": ["This is a made up review. No one cares about this review", "This is a made up review. No one cares about this review"],
    "summ2": ["This is a made up review. No one cares about this review", "This is a made up review. No one cares about this review"],
    "summ3": ["This is a made up review. No one cares about this review", "This is a made up review. No one cares about this review"]
    }
    )
    df = df.apply(lambda x: x.apply(lambda x: x.replace('\n', '') if type(x)==str else x), axis=0)
    df = df.apply(lambda x: x.apply(lambda x: x[:390] if type(x)==str and len(x)>390 else x), axis=0)
    
    df.to_csv('artifacts/amazon/gold_summs/test.csv', sep='\t', index=False)
    ...
    # Read the eval file.
    print(run_conf.output_path)
    with open(run_conf.output_path+"/out/test_eval.json") as json_file:
        data = json.load(json_file)

    rev1_summary = ' '.join(data[req1.category][req1.group_id]['gen_summ'][0])
    rev2_summary = ' '.join(data[req2.category][req2.group_id]['gen_summ'][0])

    return {
        "product1_summary": rev1_summary,
        "product2_summary": rev2_summary
    }
